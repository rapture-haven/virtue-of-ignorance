<template>
  <vue-context class="context" ref="context">
    <template slot-scope="child" v-if="child.data">
      ...
      <li>
        <span
          @click="ignore(child.data.member)"
          v-if="!child.data.member.ignored"
          >{{ $t("context.ignore") }}</span
        >
        <span @click="unignore(child.data.member)" v-else>{{
          $t("context.unignore")
        }}</span>
      </li>
      ...
      <template v-if="admin">
        <li>
          <span
            @click="mute(child.data.member)"
            v-if="!child.data.member.muted"
            >{{ $t("context.mute") }}</span
          >
          <span @click="unmute(child.data.member)" v-else>{{
            $t("context.unmute")
          }}</span>
        </li>
        <li v-if="child.data.member.id === host && !implicitHosting">
          <span @click="adminRelease(child.data.member)">{{
            $t("context.release")
          }}</span>
        </li>
        <li v-if="child.data.member.id === host && !implicitHosting">
          <span @click="adminControl(child.data.member)">{{
            $t("context.take")
          }}</span>
        </li>
        <li>
          <span
            v-if="child.data.member.id !== host && !implicitHosting"
            @click="adminGive(child.data.member)"
            >{{ $t("context.give") }}</span
          >
        </li>
      </template>
      ...
      <template v-if="admin && !child.data.member.admin">
        <li class="seperator" />
        <li>
          <span @click="kick(child.data.member)" style="color: #f04747">{{
            $t("context.kick")
          }}</span>
        </li>
        <li>
          <span @click="ban(child.data.member)" style="color: #f04747">{{
            $t("context.ban")
          }}</span>
        </li>
      </template>
...
export default class extends Vue {
  ...
  async kick(member: Member) {
    const value = await this.$swal({
      title: this.$t("context.confirm.kick_title", {
        name: member.displayname,
      }) as string,
      text: this.$t("context.confirm.kick_text", {
        name: member.displayname,
      }) as string,
      icon: "warning",
  ...
  async ban(member: Member) {
    const value = await this.$swal({
      title: this.$t("context.confirm.ban_title", {
        name: member.displayname,
      }) as string,
      text: this.$t("context.confirm.ban_text", {
        name: member.displayname,
      }) as string,
      icon: "warning",
      showCancelButton: true,
  ...
  async mute(member: Member) {
    const value = await this.$swal({
      title: this.$t("context.confirm.mute_title", {
        name: member.displayname,
      }) as string,
      text: this.$t("context.confirm.mute_text", {
        name: member.displayname,
      }) as string,
      icon: "warning",
      showCancelButton: true,
  ...
  async unmute(member: Member) {
    const value = await this.$swal({
      title: this.$t("context.confirm.unmute_title", {
        name: member.displayname,
      }) as string,
      text: this.$t("context.confirm.unmute_text", {
        name: member.displayname,
      }) as string,
      icon: "warning",
      showCancelButton: true,
