  ...
  <div ref="component" class="video">
    <div ref="player" class="player">
      <div ref="container" class="player-container">
        ...
        <div
          v-if="mutedOverlay && muted"
          class="player-overlay"
          @click.stop.prevent="unmute"
        >
      ...
      <ul v-if="!fullscreen && !hideControls" class="video-menu top">
        <li>
          <i @click.stop.prevent="requestFullscreen" class="fas fa-expand"></i>
        </li>
        <li v-if="admin">
      ...
      <ul v-if="!fullscreen && !hideControls" class="video-menu bottom">
        <li
          v-if="
            hosting && (!clipboard_read_available || !clipboard_write_available)
          "
        >
      ...
      <virtue-resolution ref="resolution" v-if="admin" />
      <virtue-clipboard
        ref="clipboard"
        v-if="
          hosting && (!clipboard_read_available || !clipboard_write_available)
...
export default class extends Vue {
  ...
  get controlLocked() {
    return (
      "control" in this.$accessor.locked &&
      this.$accessor.locked["control"] &&
  ...
  get locked() {
    return (
      this.$accessor.remote.locked ||
      (this.controlLocked && (!this.hosting || this.implicitHosting))
    );
  }
  ...
  mounted() {
    ...
    this._video.addEventListener("canplaythrough", () => {
      ...
      if (this.autoplay) {
        ...
        if (
          this.startsMuted &&
          (!document.hasFocus() || !this.$accessor.active)
        ) {
  ...
  onResize() {
    ...
    this._container.style.maxWidth = `${
      (this.horizontal / this.vertical) * offsetHeight
    }px`;
