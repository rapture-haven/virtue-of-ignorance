...
export const mutations = mutationTree(state, {
  ...
  setKeyboardModifierState(state, { capsLock, numLock, scrollLock }) {
    state.keyboardModifierState = keyboardModifierState(
      capsLock,
      numLock,
      scrollLock
    );
...
export const actions = actionTree(
  { state, getters, mutations },
  {
    ...
    changeKeyboard({ getters }) {
      ...
      $client.sendMessage(EVENT.CONTROL.KEYBOARD, {
        layout: accessor.settings.keyboard_layout,
      });
    },
    ...
    syncKeyboardModifierState(
      ...
      if (
        state.keyboardModifierState ===
        keyboardModifierState(capsLock, numLock, scrollLock)
      ) {
        return;
      }

      accessor.remote.setKeyboardModifierState({
        capsLock,
        numLock,
        scrollLock,
      });
